...
// GET helper function
function get(config, pathname, query, proxy, callback, encoding) {
  ...
  var req = protocol.request(options, function getResponse(res) {
    var data, length,
...
function api(pathname, callback, query, options) {
  ...
  if (options.dryRun) {
    ...
    get.call(this, config, pathname, query, options.proxy, function apiCallback(err, data, info) {
      if (!err) {
        try {
          if (options.parser) {
            // async parser
